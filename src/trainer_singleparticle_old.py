...
def main(
    ...
    # Create an instance of Ice_graph
    nextsim = Ice_graph(
        file_graphs,
    ...
    # Remove previous data if specified
    if remove_previous:
        if os.path.exists(save_dir):
            shutil.rmtree(save_dir)

    # Generate training and validation samples if data does not exist
    ...
    e_graph = next(iter(train_dataset))[0]
    num_features1 = e_graph.x.shape[-1]  # Node feature dimension
    v_graph = next(iter(train_dataset))[1]
    num_features2 = v_graph.x.shape[-1]  # Node feature dimension

    # Set the hidden channels and number of classes
    ...
    # Set the device and create the model
